...
OmniWall 是一个基于 Electron 的现代化桌面应用，专门用于管理和播放本地电视剧库。它提供了一个美观的海报墙界面来浏览电视剧，并内置了功能完善的播放器。此外，OmniWall 还集成了 Bangumi API，支持同步用户的 Bangumi 收藏，实现本地和在线内容的统一管理。
...
- **核心模块**:
  ...
  - `bangumi-api.js` - Bangumi API 接口，处理与 Bangumi 的数据同步
- **渲染进程**: 
  - `index.html` - 主界面，显示电视剧海报墙
  - `settings.html` - 设置页面，配置电视剧文件夹路径和 Bangumi Token
  - `player.html` - 视频播放器界面
- **前端组件**:
  - `app.js` - 应用主控制器
...
- `bangumi-set-token`: 设置 Bangumi 访问令牌
- `bangumi-get-collection`: 获取 Bangumi 收藏列表
- `bangumi-get-subject`: 获取 Bangumi 条目详情
...
1. 启动应用后，点击设置按钮配置电视剧文件夹路径和 Bangumi Token（可选）
2. 应用自动扫描并显示电视剧海报墙（包括本地内容和 Bangumi 收藏）
3. 点击海报开始播放第一季第一集
4. 在播放器中可选择不同季和集
