...
class Sidebar {
    ...
    init() {
        ...
        this.createSortMenu();
        // 延迟加载保存的排序设置，确保海报网格已初始化
        setTimeout(() => {
            this.loadSavedSortSetting();
        }, 100);
    }
    ...
    selectSortOption(sortType) {
        // 保存排序设置到本地存储
        localStorage.setItem('omniwall_sort_type', sortType);
    ...
    loadSavedSortSetting() {
        // 从本地存储加载排序设置
        const savedSortType = localStorage.getItem('omniwall_sort_type');
        if (savedSortType) {
            // 触发排序变化事件以应用保存的排序
            const event = new CustomEvent('sort-changed', {
                detail: { sortType: savedSortType }
            });
            document.dispatchEvent(event);
        }
    }
